...
<head>
    ...
    <style>
        ...
        .custom-link {
            color: inherit; /* Цвет текста черный */
...
<form method="get">
    ...
    <select name="column" class="custom-list"> <!-- Выпадающий список для выбора столбца -->
        <option value="login" <?php echo ($_GET['column'] == 'login') ? 'selected' : ''; ?>>Логин</option>
    ...
    <?php
        ...
        echo "<table border='1'>";
        echo "<tr>";
        echo "<th><a href='?sort=name_location&order=$sortOrder' class='custom-link'>Location</a></th>";
        echo "<th><a href='?sort=login&order=$sortOrder' class='custom-link'>Login</a></th>";
        echo "<th><a href='?sort=status&order=$sortOrder' class='custom-link'>Status</a></th>";
        echo "<th><a href='?sort=date_connection&order=$sortOrder' class='custom-link'>Start Date</a></th>";
        echo "<th><a href='?sort=date_shutdown&order=$sortOrder' class='custom-link'>End Date</a></th>";
        echo "<th><a class='custom-link'></a></th>";
        echo "</tr>";

        // Запрос к базе данных с учетом сортировки, поиска и условия отображения всех записей
        $sql = "SELECT places.*, clients.login FROM places JOIN clients ON places.client_id = clients.id";
        ...
        if (!$showAll) {
            if (isset($_GET['column']) && $search != '') {  
                ...
                $sql .= " WHERE ".$selectedColumn." LIKE '%$search%'";
            }
            else{
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td>" . $status . "</td>";
            echo "<td>" . (new DateTime($row['date_connection']))->format('Y-m-d') . "</td>";
            echo "<td>" . (new DateTime($row['date_shutdown']))->format('Y-m-d') . "</td>";
            echo "<td><a href='order_details.php?id=" . $row['id'] . "' class = 'custom-link'>Изменить</a></td>"; 
            echo "</tr>";
        }
        echo "</table>";
