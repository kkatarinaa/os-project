...
canvas.on("object:scaling", function (e) {  
  ...
  var angle_of_diagonal = 0;
  if (height < width) {
    angle_of_diagonal = (height**2 - width**2)/(height**2+width**2);
  }
  else{
    angle_of_diagonal = (width**2 - height**2)/(height**2+width**2);
  }

  var angle = Math.PI()/2+obj.angle/180*Math.PI()-angle_of_diagonal;
  
  if (Math.cos(angle) < Math.cos(angle_of_diagonal-angle)) {
    
  }
  else{
    if (Math.sin(angle) > Math.sin(angle_of_diagonal-angle)) {
    
    }
  }
  var left = obj.left;   
  var top = obj.top + height*Math.cos(angle);   
  var new_width = obj.width * obj.scaleX;   
  var new_height = obj.height * obj.scaleY;


  //var left_bound = limitationBox.left;   
  //var right_bound = limitationBox.left + limitationBox.width;   
  //var top_bound = limitationBox.top;   
  //var bottom_bound = limitationBox.top + limitationBox.height;
//
  //if (left < left_bound) {     
